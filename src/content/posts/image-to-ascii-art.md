...
最初用了裁切过的头像版本（`badge.webp`, 600×1200），手部被截掉了。必须用**完整原图**（`portal-figure.webp`, 1284×1590）才能保留手托地球的完整构图。

### 2. 字符宽高比

如果不除以字符宽高比（0.75），图像会被纵向拉伸 1.3 倍，人物变成瘦长条。

### 3. 亮度映射 vs 边缘检测

尝试过用 OpenCV Canny 边缘检测提取线稿，但效果反而不如直接用灰度亮度映射。对于蓝白双色线稿，亮度映射已经能很好地保留线条层次。

### 4. scaleX 的点击区域

用 CSS `scaleX(0.5)` 压扁光球视觉层时，点击区域也被缩小了。解决方法是把点击区域（透明 div）和视觉层分开，点击区域不受 `scaleX` 影响。

### 5. 性能

37000+ 个 `<span>` 元素 + 多层 CSS 动画会导致卡顿。优化方向：
- 减少 `text-shadow` 层数
- 减少 CSS 动画元素数量
- 用 `will-change: transform` 触发 GPU 加速
- 避免持续运行的 `requestAnimationFrame`（只在需要时启动）
